...
export class ColumnSizeService extends BeanStub implements NamedBean {
    ...
    public refreshFlexedColumns(
        ...
        const freeze = (item: FlexItem, width: number) => {
            ...
            frozenSpace += width;
            freeSpace = totalSpace - frozenSpace;
        ...
        while (unfrozenItemCount > 0) {
            ...
            const spaceToFill = Math.round(unfrozenFlex < 1 ? unfrozenFlex * freeSpace : freeSpace);

            // c. Distribute free space proportional to the flex factors.
            let remainingSpaceToFill = spaceToFill;
            for (const item of items) {
                if (isFrozen(item)) continue;
                if (item !== items[items.length - 1]) {
                    remainingSpaceToFill -= item.targetSize = Math.round(spaceToFill * (item.flex / unfrozenFlex));
                } else {
                    // Avoid cumulative rounding errors by ensuring last item
                    // always uses all unallocated space
                    item.targetSize = remainingSpaceToFill;
                }
            }
        ...
        // TODO: Question: this preserves behaviour from previous versions, but
        // is it correct? Should we be returning a list of flex columns, rather
        // than unconstrained flex columns?
        const flexingColumns = items.filter((item) => item.isFlex && !item.violationType).map((item) => item.col);
